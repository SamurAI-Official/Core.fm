...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'Core.fm API' });
});
...
app.get('/song/:id', async (req, res) => {
  ...
  try {
    ...
    const description = `🎵 ${song.style} • Create your own AI music free on Core.fm`;
...
<body>
  <p>Redirecting to <a href="${config.frontendUrl}?song=${song.id}">Core.fm</a>...</p>
</body>
</html>`);
  } catch (error) {
...
app.listen(config.port, '0.0.0.0', () => {
  console.log(`Core.fm Server running on http://localhost:${config.port}`);
