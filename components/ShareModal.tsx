...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, song }) => {
  ...
  const defaultShareText = `🎵 "${song.title}" ${song.style ? `(${song.style})` : ''} - Made with Core.fm`;
  const twitterText = `🔥 Just created "${song.title}" with Core.fm - local AI music generation! ${song.style ? `#${song.style.replace(/\s+/g, '')}` : ''} #AIMusic #ACEStep`;
  const redditTitle = `[AI Music] ${song.title} - ${song.style || 'Original'} | Created with Core.fm`;
  const whatsAppText = `🎧 Listen to this AI-generated song!\n\n"${song.title}" by ${song.creator || 'Unknown Artist'}\n${song.style ? `Genre: ${song.style}` : ''}\n\nMade with Core.fm - free and open source!`;
